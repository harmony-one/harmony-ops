...
addresses=$(curl -sL https://bit.ly/pga-keys | tac | sed -e '/var /q' |\
...
raw=$(echo "$addresses" | xargs -P 100 -i{} bash -c \
'address=$(echo "{}" | cut -d " " -f 1); shard=$(echo "{}" | cut -d " " -f 2);
result=$(wallet_files/wallet -p pangaea balances -address="$address");
