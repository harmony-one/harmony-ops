...
URL_update_db = 'http://api_key:' + API_SECRET + \
    '@18.144.50.152:3000/api/dashboards/db'
    ...
    for ind in range(4):
        ...
        for idx in range(ip_array_size):
            ...
            for idy in range(4):
                ...
                cpu_query = "100 - (avg by (instance) (irate(node_cpu_seconds_total{instance=\""+ip+':9100\",job=\"'+shard+'\",mode=\"idle\"}[5m])) * 100)'
                ram_query = "(node_memory_MemTotal_bytes{instance=\""+ip+':9100\",job=\"'+shard+'\"} - node_memory_MemFree_bytes{instance=\"'+ip.rstrip()+':9100\",job=\"'+shard+'\"}) / node_memory_MemTotal_bytes{instance=\"'+ip.rstrip()+':9100\",job=\"'+shard+'\"} * 100'
                memory_actual_usage_query = "(node_memory_MemTotal_bytes{instance=\""+ip+":9100\"} - node_memory_MemAvailable_bytes{instance=\""+ip+":9100\"}) * 100/ node_memory_MemTotal_bytes{instance=\""+ip+":9100\"}"
                disk_query = "node_filesystem_avail_bytes{instance=\""+ip+':9100\",job=\"'+shard+'\", mountpoint="/"}/1024/1024/1024'
                network_incoming_query = "rate(node_network_receive_bytes_total{instance=\""+ip+":9100\", device=\"eth0\"}[5m]) / 1024"
                network_outgoing_query = "rate(node_network_transmit_bytes_total{instance=\""+ip+":9100\", device=\"eth0\"}[5m]) / 1024"
                ...
                # SECOND COLUMN - RAM Monitoring
                    ...
                    data_cpu_insert["targets"][0].update({"legendFormat" : "Occupied Memory"})
                    data_cpu_insert["yaxes"][0].update({"label": "%"})
